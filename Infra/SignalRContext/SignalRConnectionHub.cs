...
public class SignalRConnectionHub : Hub
{
    ...
    public async Task SendMessageToUser(string targetId, string message)
    {
        try
        {
            ...
            // Participants
            Participants sender = await _participantsRepository.GetParticipants(senderGuid, cancellationToken); 
            Participants target = await _participantsRepository.GetParticipants(targetGuid, cancellationToken); 
        
            // 1) Check is an participants chat exists (Created when the users became friends.)
            Chats? chat = await _chatRepository.SearchChatByParticipants(sender.Id, target.Id, cancellationToken);
            if(chat == null)
                throw new HubException("Invalid chat participants");
        
            // // 2) check if destination user exists. Really need it??
            ...
            await _backgroundTaskQueue.QueueBackgroundWorkItemAsync(async ct =>
            {
                ...
                try
                {
                    Messages msg = Messages.CreateMessage(chat.Id, sender.Id, message, DateTime.UtcNow);
        ...
        catch (Exception ex)
        {
            ...
            throw new Exception("Hub message error");
        }
    }
}